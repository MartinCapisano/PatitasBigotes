...
| <a id="B-07"></a>**B-07** | `update_product(active=...)` propaga a **todas** las variantes; reactivar pierde el estado individual | `products_s::update_product` | 🟠 Pérdida silenciosa de configuración de surtido | 0,5 d | **P1** |
| <a id="B-08"></a>**B-08** | `POST /admin/orders/{id}/payments/manual` exige un pago pendiente **del mismo método** | `payment_s::confirm_manual_payment_for_order` | 🟠 No se puede registrar una transferencia si el cliente eligió Mercado Pago | 0,5 d | **P1** |
...
| <a id="R-11"></a>**R-11** | **Unificar la estrategia transaccional** — ⚠️ *matizado por [ADR 0002](adr/0002-estrategia-transaccional-endpoints-idempotentes.md)*: la unificación **ingenua** (colapsar la dependencia de sesión por sí misma) ya se descartó a conciencia — cada patrón es la aplicación correcta del árbol [decision-sesion-transaccional.mmd](diagrams/decision-sesion-transaccional.mmd), no deuda accidental. Lo que queda es la unificación **con principio**: sacar el trabajo post-commit de los handlers (middleware / cierre de dependencia) y recién ahí reconsiderar `get_db` vs `get_db_transactional`. **Depende de R-04** (emails post-commit) | 3 patrones conviviendo en `orders_r.py` | 🟠 | 2 d | **P2** |
